// ConvMd2JsExt.go
// program that converts markdown files into jsDom files
// ./simpleMdCon /in=infile /out=outfile [/dbg]
...
//log.Fatalf("error -- no output file name provided!\n")}
	...
	if len(metaData) > 0 {
		mData, err := jsdom.GetMeta(metaData)
		if err !=nil {log.Fatal("error -- converting meta: %v\n", err)}
		jsdom.PrintMeta(mData)
	}

	startMdStr := jsdom.JSRenderStartFunc()
	...
	name:= "jsDom"
	jsDomRen := jsdom.GetRenderer(name, dbg)
//	md := goldmark.New(goldmark.WithExtensions(extDom.GFM))

	md := goldmark.New(goldmark.WithRenderer(jsDomRen), goldmark.WithExtensions(extDom.Table,))

// the extension depends on the jsDomren rnederer thus it has to be defined with teh extension!!
//	md.SetRenderer(jsDomRen)
